...
class MerPurchaseBudget(models.Model):
    ...
    @api.model
    def _refresh_usage_metrics(self):
        """Trigger recomputation for all active and draft budgets."""
        budgets = self.search([("state", "in", ("draft", "active"))])
        if budgets:
            budgets._compute_amounts()
        return True

    def action_activate(self):
        for budget in self:
            if budget.budget_amount <= 0:
                raise UserError(_("Số tiền ngân sách phải lớn hơn 0 trước khi kích hoạt."))
