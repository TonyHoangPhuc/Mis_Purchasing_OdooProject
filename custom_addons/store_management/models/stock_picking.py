...
class StockPicking(models.Model):
    ...
    mer_request_id = fields.Many2one(
        "mer.purchase.request", string="Yêu cầu Merchandise", compute="_compute_mer_request_id", store=True, readonly=False
    )
    is_central_picking = fields.Boolean(compute='_compute_warehouse_role_flags', string="Là phiếu Kho tổng")
    is_store_picking = fields.Boolean(compute='_compute_warehouse_role_flags', string="Là phiếu Cửa hàng")

    @api.depends('picking_type_id.warehouse_id.mis_role')
    def _compute_warehouse_role_flags(self):
        for picking in self:
            role = picking.picking_type_id.warehouse_id.mis_role
            picking.is_central_picking = (role == 'central')
            picking.is_store_picking = (role == 'store')
    ...
    def _is_qc_managed_receipt(self):
        self.ensure_one()
        # Chỉ Cửa hàng mới cần quản lý quy trình QC, Kho tổng chỉ cần Xác nhận trực tiếp
        return self._is_store_receipt_for_qc()
    ...
    def _compute_store_receiving_context(self):
        ...
        for picking in self:
            ...
            elif picking.mer_request_id:
                # Bất kể là incoming hay outgoing, hễ có PR là theo luồng Kho tổng -> Cửa hàng
                route_type = "central_to_store"
                route_label = _("Kho tổng -> Cửa hàng")
                source_party = _("Kho tổng")
                if picking.picking_type_code == "incoming" and picking.picking_type_id.warehouse_id.mis_role == "store":
                    origin_wh = picking.move_ids.mapped("move_orig_ids.picking_id.picking_type_id.warehouse_id")[:1]
                    if origin_wh:
                        source_party = origin_wh.name
                source_document = picking.mer_request_id.name or picking.origin or picking.name
    ...
    def action_start_actual_check(self):
        for picking in self:
            picking.store_actual_check_done = True
            if picking.wm_qc_status == 'draft':
                picking.action_start_qc()
        return True
    ...
    def action_complete_central_receipt_check(self):
        for picking in self:
            ...
            # Tự động điền số lượng nhận bằng số lượng dự kiến nếu chưa nhập
    ...
    def action_qc_pass(self):
        ...
        if remaining_pickings:
            ...
            # Chuyển thẳng sang 'done' thay vì 'passed'
            remaining_pickings.write({"wm_qc_status": "done"})
    ...
    def button_validate(self):
        # Chỉ chặn validate nếu là đơn Cửa hàng nhận từ Kho tổng mà chưa qua bước QC
        pickings_requiring_qc = self.filtered(
            lambda p: p.picking_type_code == 'incoming' 
            and p.store_route_type == 'central_to_store' 
            and p.wm_qc_status not in ('passed', 'done', 'checking') # Cho phép validate nếu đang ở bước kiểm tra
            and p.state not in ('done', 'cancel')
        )
        if pickings_requiring_qc:
            raise UserError(_("Vui lòng hoàn tất kiểm tra chất lượng (QC) trước khi xác nhận nhập kho."))
            
        result = super().button_validate()
        completed_pickings = self.filtered(lambda picking: picking.state == "done")
        if completed_pickings:
            # Chuyển sang cột "Đã hoàn tất" để phân loại trong Kanban
            completed_pickings.write({'wm_qc_status': 'done'})
