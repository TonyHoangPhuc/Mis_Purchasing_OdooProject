...
class StockPicking(models.Model):
    ...
    def button_validate(self):
        ...
        completed_pickings = self.filtered(lambda p: p.state == 'done')
        if completed_pickings:
            # Nếu phiếu nhập chưa qua QC (vẫn là draft) mà đã validate xong (ví dụ Kho tổng xác nhận trực tiếp)
            # thì tự động chuyển sang 'done' để kích hoạt tạo hóa đơn
            completed_pickings.filtered(lambda p: p.wm_qc_status == 'draft').write({'wm_qc_status': 'done'})
            completed_pickings._wm_trigger_post_receipt_actions()
        return res

    def _wm_trigger_post_receipt_actions(self):
        """Hàm dùng chung để tạo hóa đơn và khóa PO sau khi nhập kho"""
        for picking in self:
            ...
            if picking.purchase_id:
                try:
                    # Sử dụng sudo và kiểm tra đơn giản để tránh kích hoạt các hàm compute lỗi của core Odoo
                    po = picking.purchase_id.sudo()
                    
                    # Cố gắng tạo hóa đơn luôn, nếu Odoo cho phép thì sẽ tạo thành công
                    existing_invoices = po.invoice_ids
                    po.with_context(create_bill=True).action_create_invoice()
                    new_invoices = po.invoice_ids - existing_invoices
                    
                    if new_invoices:
                        warehouse = picking.location_dest_id.warehouse_id
                        store = self.env['store.store'].search([('warehouse_id', '=', warehouse.id)], limit=1)
                        update_vals = {'invoice_date': fields.Date.today(), 'ref': picking.name}
                        if store:
                            update_vals['store_id'] = store.id
                        new_invoices.write(update_vals)
                        
                        picking.purchase_id.message_post(
                            body=_("Hệ thống đã tự động tạo hóa đơn NCC %s từ phiếu nhập kho %s.") % (
                                ", ".join(new_invoices.mapped('name') or ['Nháp']), picking.name
                            )
                        )
                except Exception as e:
                    # Nếu lỗi, ghi lại log để theo dõi thay vì chặn đứng quy trình nhập kho
                    picking.purchase_id.message_post(body=_("Lưu ý: Hệ thống chưa thể tạo hóa đơn tự động ngay. Lý do: %s") % str(e))
            
            # 3. Tự động Hoàn tất PO nếu đã nhận đủ hàng
            if picking.purchase_id:
    ...
    def action_qc_pass(self):
        ...
        if pickings_to_validate:
            ...
            # Logic tạo hóa đơn đã được gọi trong button_validate override ở trên
            return res
